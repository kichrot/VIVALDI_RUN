 ...
 * Read the description of this JS mod in the User's guide for the "Vivaldi_RUN" utility version 0.2.1.0 and higher.
 * Описание данного JS-мода читать в Руководстве пользователя для утилиты "Vivaldi_RUN" версии 0.2.1.0 и выше.
...
ModPanelHidden = function(e) {
	if ( document.querySelector(ModPanelSel) ) {
    	ModPanelcssText=ModPanelcssText_0;
		document.querySelector(ModPanelSel).style.cssText = ModPanelcssText; //Скрываем панель
    	...
    	/*document.querySelector("#ButtonModPanel > button > svg").style.zoom = "130%";*/
	}
};

function CreateNewButtonPanel(indexNewPanel, toolbarNewButton, titleNewButton, svgNewButton, orientationNewPanel) {
    ...
    function openonmouseout() {
 	...
 	function openClick() {
 		...
 		var ButtonModPanelSel = '#ButtonModPanel'+'_'+indexNewPanel;
 		if (ModPanelSel != '' && ModPanelSel != 'div#ModPanel'+'_'+indexNewPanel)  {
 			document.querySelector(ModPanelSel).style.cssText = ModPanelcssText; //Скрываем другую открытую панель
 		}		
 		ModPanelSel = 'div#ModPanel'+'_'+indexNewPanel;
 		if (document.querySelector(ModPanelSel).style.visibility == "hidden") {
 			document.querySelector(ModPanelSel).style.cssText = ModPanelcssText;
 			if (document.documentElement.clientHeight < (document.querySelector(ModPanelSel).getBoundingClientRect().top + document.querySelector(ModPanelSel).offsetHeight)) {
 		   		var ModPanelOffsetHeight=0-(document.querySelector(ModPanelSel).offsetHeight-document.querySelector(ButtonModPanelSel).offsetHeight)-30;
 		   		ModPanelcssText=ModPanelcssText+('top: '+String(ModPanelOffsetHeight)+'px;');
 			}
 			if (document.documentElement.clientWidth < (document.querySelector(ModPanelSel).getBoundingClientRect().left + document.querySelector(ModPanelSel).offsetWidth) + 30) {
 		   		var ModPanelOffsetWidth=0-(document.querySelector(ModPanelSel).offsetWidth-document.querySelector(ButtonModPanelSel).offsetWidth);
 		   		ModPanelcssText=ModPanelcssText+('left: '+String(ModPanelOffsetWidth)+'px;');
 			}
 		} else { 
            document.querySelector(ModPanelSel).style.cssText = ModPanelcssText + 'visibility: visible;';
        }
 		if (document.querySelector(ModPanelSel).style.visibility == "hidden") { 
            document.querySelector(ModPanelSel).style.cssText = ModPanelcssText + 'visibility: visible;'; //Показываем панель
            /*document.querySelector("#ButtonModPanel > button > svg").style.transform = "scale(0.7)";*/
            /*document.querySelector("#ButtonModPanel > button > svg").style.zoom = "115%";*/
        } else { 
            ModPanelHidden();
        }
    }
    ...
    function NewButton () {
        ...
        outer_div.id = 'ButtonModPanel'+'_'+indexNewPanel;
        ...
        ModPanel.id = 'ModPanel'+'_'+indexNewPanel;
...
function CreateNewButton(KEYPRESS_NewButton, classNewButton, toolbarNewButton, titleNewButton, svgNewButton, hidePanel) {
...
//////////////////////////////* Кнопки создания и открытия новой панели / Buttons for creating and opening a new panel *///////////////////////////////////
/* You can create multiple buttons and appropriate panels for placing the buttons you create. Read more about this in the User Guide. */
/* Вы можете создать несколько кнопок и соответствующих панелей для размещения создаваемых вами кнопок. Узнайте больше об этом в Руководстве пользователя. */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

var NewPanelIndex='1';
var NewButtonToolbar = '.UrlBar.toolbar.toolbar-mainbar.toolbar-large';
...
var NewButtonTitle = 'Панель Vivaldi_RUN / Vivaldi_RUN panel';
var NewButtonSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#205AFC"><path transform="rotate(90 12,12) " id="svg_1" d="m12,0c-6.627,0 -12,5.373 -12,12s5.373,12 12,12s12,-5.373 12,-12s-5.373,-12 -12,-12zm-3,18l0,-12l9,6l-9,6z"/></svg>';
var NewPanelOrientation = 'horizontally';
//var NewPanelOrientation = 'vertically';
CreateNewButtonPanel(NewPanelIndex, NewButtonToolbar, NewButtonTitle, NewButtonSvg, NewPanelOrientation);

 
///////////////////////////////////////////// Кнопки на новой панели 1 / Buttons on the new panel 1 

/* Кнопка "Перезагрузка VIVALDI" / Button "Restart VIVALDI" */
var NewButton_KEYPRESS = '"0"'; 
var NewButtonClass = 'mod-ReloadVIVALDI';
var NewButtonToolbar = 'div#ModPanel_1';
...
var PanelHide = 'Y';
CreateNewButton(NewButton_KEYPRESS, NewButtonClass, NewButtonToolbar, NewButtonTitle, NewButtonSvg, PanelHide);
...
//var NewButton_KEYPRESS = '"11" |C:\\Program Files (x86)\\AkelPad\\AkelPad.exe| <Y>';
var NewButton_KEYPRESS = '"11" |C:\\Program Files\\Vivaldi\\Application\\Shortcuts for Vivaldi_Run\\AkelPad.lnk| ';
...
var NewButton_KEYPRESS = '"11" |C:\\Windows\\System32\\calc1.exe| <Y>';
...
/* Кнопка "VivaldiModManager" */ 
var NewButton_KEYPRESS = '"11" |C:\\Program Files\\Vivaldi\\VivaldiModManager\\VivaldiModManager.exe| <Y>'; 
var NewButtonClass = 'mod-VivaldiModManager';
var NewButtonToolbar = 'div#ModPanel_1';
var NewButtonTitle = 'VivaldiModManager';
var NewButtonSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="24pt" height="24pt" viewBox="0 0 24 24" fill="#EA4333"><g transform="translate(0,24) scale(0.1,-0.1)"><path d="M0 120 l0 -120 120 0 120 0 0 120 0 120 -120 0 -120 0 0 -120z m200 75 c31 -38 -27 -96 -65 -65 -15 13 -21 61 -8 73 12 13 60 7 73 -8z m-47 -94 c4 0 7 -8 7 -17 0 -10 6 -14 15 -10 21 8 19 -21 -2 -32 -25 -12 -101 -7 -118 8 -19 16 -19 34 0 26 8 -3 19 3 25 14 9 17 21 19 73 11z"/></g></svg>';
var PanelHide = 'Y';
CreateNewButton(NewButton_KEYPRESS, NewButtonClass, NewButtonToolbar, NewButtonTitle, NewButtonSvg, PanelHide);
...
/* Кнопка включения/отключения автоскрытия панели задач / Button to enable/disable auto-hide taskbar */
var NewButton_KEYPRESS = '"22"'; 
var NewButtonClass = 'mod-taskbar-status';
var NewButtonToolbar = '.UrlBar.toolbar.toolbar-mainbar.toolbar-large';
var NewButtonTitle = 'Включение/отключения автоскрытия панели задач / Enable/disable auto-hide taskbar';
var NewButtonSvg = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#b12e6a"><path d="M24 13h-2v9h-9v2h11v-11zm-24-13h11v2h-9v9h-2v-11zm20.622 7.621l-13 13 3.378 3.379h-11v-10.999l3.378 3.378 13-13-3.378-3.379h11v10.999l-3.378-3.378z"/></svg>';
var PanelHide = 'Y';
//CreateNewButton(NewButton_KEYPRESS, NewButtonClass, NewButtonToolbar, NewButtonTitle, NewButtonSvg, PanelHide);
